...
const calculateStreak = (entries: { created_at: Date }[]): number => {
    ...
    const dates = entries.map((e) => new Date(e.created_at).toLocaleDateString("en-CA")).sort().reverse();
...
const sidebarLinks = [
    ...
    { id: "3", icon: Calendar, label: "Reports", to: "/dashboard/reports" }
]
export default function Sidebar() {
    ...
    const streakMessage = streak < 2 ? `${streak} day` : `${streak} days`

    return (
        <aside className="w-full border-r h-screen bord dark-dim pt-[2rem]">
            <div className="mx-4 mt-[2.5rem] flex flex-col justify-between h-[calc(100vh-4.5rem)]">
                <div>
                    <h3 className="text-[0.7rem] text-dim ml-3 font-inter tracking-widest">MAIN</h3>
                    <div className="mt-2">
                        {sidebarLinks.map((s) => (
                ...
                <div className="flex flex-col border-t bord pt-3 pb-6 font-inter">
